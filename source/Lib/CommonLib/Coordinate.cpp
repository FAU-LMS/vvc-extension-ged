...
Array2TCoord FloatingFixedConversion::fixedToFloating(const Array2Fixed &fixed, int precision) {
  Array2TCoord floating = fixed.unaryExpr([precision](int val){ return TCoord(val >> precision); });
...
Array2Fixed FloatingFixedConversion::floatingToFixed(const Array2TCoord &floating, int precision) {
